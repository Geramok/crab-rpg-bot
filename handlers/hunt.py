...
async def perform_search(message: Message, state: FSMContext):
    ...
    if barrier:
        ...
        barrier_line = (
            ...
            f"уровней мутаций (сейчас {have}) — дальше не пройти."
        )
...
def _do_combat_round(target, stats, specials, log, force_crit=False, guaranteed_hit=False,
    ...
    def do_hit(force_crit=force_crit):
        ...
        if "vampirism" in specials:
            ...
            max_heal = round(stats["max_hp"] * 0.10)  # Жесткий потолок: 10% от макс. ХП
            heal = min(heal, max_heal)
            if heal > 0:
                log.append(f"🩸 +{format_number(heal)} HP")
                do_hit.heal = do_hit.__dict__.get("heal", 0) + heal
                
        if "poison" in specials and random.random() * 100 < 15:  # Шанс снижен до 15%
    ...
    puncture_crit = "puncture" in specials and random.random() * 100 < 10  # Шанс снижен до 10%
    was_crit = do_hit(force_crit=(force_crit or puncture_crit))

    if (target.get("is_boss") or target.get("hp", 0) > 0) and was_crit and "frenzy" in specials and random.random() * 100 < 20:  # Шанс снижен до 20%
...
def _resolve_monster_counter(target, stats, specials, log, evasion_penalty=0, block_percent=0, guaranteed_dodge=False):
    ...
    if not dodged and not target.get("is_boss") and "camouflage" in specials and random.random() * 100 < 10:  # Шанс снижен до 10%
...
async def _finish_turn(call, user_id, fsm_data, state: FSMContext, is_camp, target, log):
    ...
    if not target.get("is_boss") and target.get("hp", 1) <= 0:
        ...
        if is_camp:
            ...
            if "greed" in specials and random.random() * 100 < 15:  # Шанс снижен до 15%
                total_gold *= 2
            ...
            await state.set_data({})
            await _push_battle_update(message, user_id, msg_id, final_text)
            await message.answer("Готов к новому ры