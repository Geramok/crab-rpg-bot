...
# Лагеря стражей (3 элитных существа разом) убраны по просьбе — вместо них
# каждые ELITE_ENCOUNTER_INTERVAL метров попадается ОДИН чуть усиленный
# обычный монстр (та же механика роллинга, просто с множителем силы и
# золота). Мультипликатор специально мягкий — раз это происходит часто
# (интервал 10м близок к размеру обычного шага 1-15м), сильный баф ощущался
# бы утомительным на каждом втором бою.
GUARD_CAMP_INTERVAL = 100  # оставлено для совместимости, лагеря больше не запускаются
GUARD_CAMP_ELITE_MULT = 1.25
ELITE_ENCOUNTER_INTERVAL = 10
ELITE_ENCOUNTER_MULT = 1.12
